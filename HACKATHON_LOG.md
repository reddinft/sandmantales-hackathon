# Sandman Tales — Hackathon Build Log
_Team ClawCutters | Mistral AI Hackathon | UNSW Founders, Sydney | Feb 28–Mar 1, 2026_
...
#### 14:00–15:00 AEST — Planning & Agent Setup
- Created 4 Mistral Agents via API: Doc (orchestrator), Pathfinder (story gen), Firefly (builder), Lifeline (voice/audio)
- Each agent has a distinct personality and accent: Doc (Trinidad 🇹🇹), Pathfinder (Samurai 🇯🇵), Firefly (Australian 🇦🇺), Lifeline (Kiwi 🇳🇿)
- Wrote SOUL.md personality files for all 4 agents
- Agent IDs stored in `mistral_agents.json`
- **Decision:** Fresh repo `reddinft/sandmantales-hackathon` — no legacy code from old project

#### 15:00–15:30 — The Orchestration Problem (The Gotcha)
- Spawned Doc as lead orchestrator. He was supposed to delegate to Pathfinder, Firefly, Lifeline via Mistral Agents API
- **Problem:** Doc tried to do EVERYTHING himself. Wrote code, generated content, made architecture decisions — never called his team agents once
...
- **Fix:** Built `team.py` — a CLI tool (generated by Vibe CLI/devstral-2) that lets Doc call any agent with `python3 team.py pathfinder "message"`
...
#### 15:30–16:00 — Phase 1: Agent Outputs
- **Pathfinder** → Generated "Luna the Brave Little Fox" (6 scenes, structured JSON with title, text, mood, illustration_prompt per scene)
- **Firefly** → Delivered 26-file flat architecture plan for FastAPI + React
- **Lifeline** → Researched ElevenLabs multilingual voice config: recommended voices, models, stability/similarity settings per language (EN/JA/FR/HI)
- **Doc** → Created 11 GitHub issues, started backend scaffold
- All communication via Mistral Conversations API — provably Mistral-generated

#### 16:00–16:30 — Phase 1 Demo Video
...
#### 16:30–16:50 — Doc Phase 2: The Build
- Respawned Doc with explicit step-by-step build instructions
- Doc used **Vibe CLI (devstral-2)** for ALL code generation — not Claude, not GPT
...
#### 16:50–17:15 — E2E Testing & Bug Fixes
Critical bugs found and fixed during testing:

1. **Mistral FunctionCallEntry bug:** Pathfinder has a function tool defined, so instead of returning text content, the API returns a `FunctionCallEntry` with story data in `.arguments`. Both `team.py` and `main.py` assumed `.content` existed → crashed with `'FunctionCallEntry' object has no attribute 'content'`
...
2. **aiosqlite threading bug:** `async with await get_db() as db:` was reusing the same `aiosqlite.connect()` object, which starts an internal thread. Second call → `RuntimeError: threads can only be started once`
...
3. **Mistral SDK + asyncio incompatibility:** Even after fixing aiosqlite, calling the Mistral Python SDK inside a FastAPI async endpoint caused threading conflicts
   - **Fix:** Shell out to `team.py` via `asyncio.create_subprocess_exec()` — the SDK works fine in its own process
...
6. **ElevenLabs voice_not_found:** Lifeline recommended voice IDs that exist in ElevenLabs' shared library but weren't added to our account. JA/FR/HI voices returned 404
   - **Fix:** Use Adam (premade, available in every account) with `eleven_multilingual_v2` model — it speaks all languages perfectly
...
- ✅ POST /api/story → Pathfinder → Mistral Agents API
- ✅ GET /api/stories (list)
- ✅ GET /api/stories/:id (detail)
- ✅ POST /api/narrate → ElevenLabs (95KB MP3)
- ✅ Frontend dev server on :5173

#### 17:15–17:40 — Demo Stories & Illustrations
- Generated 3 demo stories via the live API:
  - **EN:** "Mia and the Glowing Garden of Whispers" (6 scenes)
  - **JA:** "月明かりの約束：ゆきと小さなきつねの冒険" (6 scenes)
  - **FR:** "Sophie et la Baleine de Nuages" (6 scenes)
- All stories generated by Pathfinder via Mistral Conversations API — fully traceable
...
- Tailwind v4 → v3 downgrade required (v4 uses different config format)
...
#### 17:40–18:00 — FLUX Fine-Tuning Pipeline
- **Doc researched and delivered FINETUNE_PLAN.md** via team.py (Firefly consultation)
- Decision: LoRA approach (not DreamBooth/full fine-tune) — fits 24GB M4
...
- **Automated pipeline:** Image gen → auto-detect completion → stop Ollama → train → restart Ollama
...
User → React Frontend (Vite + Tailwind)
     ...
     team.py → Mistral Conversations API → Pathfinder Agent
         ↓ FunctionCallEntry.arguments (structured story JSON)
...
Illustrations: FLUX schnell (local, mflux on Apple Silicon M4)
                → LoRA fine-tuned for storybook style
```

## Agent Communication Flow
```
Doc (orchestrator)
  → team.py pathfinder "Generate story for Luna..."
    → Mistral Conversations API
    → Pathfinder agent processes
    → FunctionCallEntry with create_bedtime_story(title, scenes)
    → team.py parses .arguments, outputs JSON
  → main.py reads stdout, saves to SQLite

Doc
  → team.py lifeline "What voice settings for multilingual TTS?"
    → Lifeline agent researches
    → MessageOutputEntry with recommendations
    → team.py outputs text

Doc
  → team.py firefly "What's the architecture for this app?"
    → Firefly agent designs
    → MessageOutputEntry with 26-file plan
```
...
- **0** lines of code written by Claude/GPT — ALL via Mistral (Vibe CLI + Agents API)
...
- **FunctionCallEntry pattern:** Pathfinder's `create_bedtime_story` function returns structured data — more reliable than asking for JSON in text
...
## Quotable Moments
- "An orchestrator without frictionless delegation tools is just a fancy single agent." — learned when Doc refused to use his team
- "We built team.py to make delegation easier than doing it yourself." — the fix
- "The bug wasn't the Mistral SDK, it was aiosqlite creating threads inside our async event loop." — 20 minutes of misdirected debugging
- "Pathfinder doesn't return text — it calls a function. The story data is in the function arguments, not the content." — Mistral Agents with tools behave differently
- "Adam speaks Japanese perfectly. You don't need language-specific voices with eleven_multilingual_v2." — ElevenLabs discovery
